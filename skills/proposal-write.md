...
1. **Voice pack location:** The `@~/proposal-resources/voice/` references point to writing style files. Create your own with sentence length preferences, hedging rules, and formatting conventions, or remove these lines to use general academic voice.

2. **Folder paths** (Steps 2 and 8): The paths `05_Submissions/Grants/`, the weekly-reviews directory, and `~/proposal-resources/donors/` reflect one folder naming convention. Update all paths to match your own project structure.
