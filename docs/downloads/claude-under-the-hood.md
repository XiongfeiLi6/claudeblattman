...
```
1. Read ~/.claude/CLAUDE.md (symlinked to ~/Documents/Claude-Settings/CLAUDE.md)
...
Your skills explicitly say "DO NOT use Task agents" — and that's correct for those skills, because Task agents trigger permission prompts and add latency. But for ad-hoc research tasks ("find all emails from a collaborator about the project decision"), a Task agent is the right call because it keeps the main window clean.
...
**Advanced technique: Eval-driven refinement.** Create a file at `~/Documents/assistant-workflow/eval/triage-eval-cases.md` with 10 representative emails (5 clear, 5 borderline). After any rule change, run `/triage-inbox noapply` and manually verify the 5 borderline cases match expected classifications. This takes 5 minutes and prevents regressions.
