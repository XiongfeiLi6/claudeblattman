...
- `~/Documents/assistant-workflow/policies/` — VIP lists, skip-inbox rules, calendar IDs
- `~/Documents/assistant-workflow/config/` — Label IDs, vendor domains, classification rules
...
```bash
echo "$(date +%Y-%m-%d),SKILL_NAME,TOOL_CALLS,NOTES" >> ~/Documents/Claude-Settings/logs/skill-performance.csv
```
...
- **Standard skills**: `~/Documents/Claude-Settings/commands/` (symlinked to `~/.claude/commands/`)
- **Domain-specific skills**: Can live in their own directories (e.g., `~/Documents/proposal-resources/skills/`) and be symlinked into `commands/`. Use this pattern for skills that:
...
- **Current symlinked skills**: `write-proposal`, `revise-proposal`, `donor-profile` (all from `proposal-resources/skills/`)
